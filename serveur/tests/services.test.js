...
// REQUIRE
const {
  MongoDbOpenConnexion,
  MongodbCreateCollections,
  MongodbDeleteDatabase,
  MongoDbCloseConnexion,
  ...coll
} = require("../src/conn-databases/mongodb");
...
test(
  ...
  async () => {
    ...
    const init_stock_result_1 = await init_stock([
      "https://facebook.com",
    ...
    const init_stock_result_2 = await init_stock([
      "https://facebook.com",
      "https://usherbrooke.ca",
      "https://en.wikipedia.org",
      "https://expressjs.com",
    ...
    const init_stock_result_3 = await init_stock([
      "https://facebook.com",
...
test(
  "client stock url two time with sames and differents url",
  async () => {
    // Subscribe
    const crawler_token = await subscribe(process.env.CRAWLER_TOKEN_SECRET);
    expect(crawler_token).toBeTruthy();
...
test(
  ...
  async () => {
    ...
    const checkLevel = (level, url_name) => {
      return (
        graph_one_node_result.edges
          .filter((x) => x.numConnections === level)
          .map((x) => x.url_parent)
          .indexOf(url_name) > -1
      );
    };
